...
1. **PageUp/PageDown 完全缺失** —— 用户明确要求（`views/key_bindings.py` 无 `pageup`/`pagedown` 分支，已核验）。
2. **Ctrl+Home/Ctrl+End 实现为「行首/行尾」而非「文档首/尾」** —— `move_line_start`/`move_line_end`（`views/editor.py` L600-L625）与普通 Home/End（L531-L550）同效，仅 `_nav_blocked` 检查不同，行为不符合预期。
3. **导航后光标不滚动入视** —— 只有 TOC `jump_to`（L1783-L1802）调 `scroll_to`；上/下/Home/End/点击移动光标后，光标可能离开视口而不自动跟随，体感「光标消失」。
4. **上/下行移动无「记忆列」** —— `move_up`/`move_down`（L561-L598）每次用当前 `line_offset` 作目标列，跨短行即被夹断丢失原列。
5. **点击命中不做中点吸附** —— `_hit_test_segs`（L95-L124）以「字符左边界」为阈值（`measure_text_width(display[:j]) >= local_x`），点击左半字符也落在其后，且最左边缘点击偏移到第 1 字符之后（off-by-one）。
6. **拖拽跨行选区不跟踪 x** —— `_pan_target_off`（L727-L741）跨行时硬编码 999999（行尾）/0（行首），不按鼠标 x 列定位目标行偏移，体感粗糙。
...
- `views/line_view.py`：`_hit_test_segs` L95-L124、`_hit_test_code` L127-L161、`_pan_target_off` L727-L741、`LineView` L294-L323、`_hit_test_tap` L86 —— 均如计划所述；`_hit_test_x` 与 `on_hit_test_x` prop **尚不存在**，需新增。
- `views/editor.py`：`move_up` L561-L598、`move_down` L581-L598、`move_home` L531-L550、`move_end` L541-L550、`move_line_start` L600-L611、`move_line_end` L613-L625、`_goto`/`activate` L426-L445、`_goto_quiet` L458-L472、`_locate_seg_by_raw_offset` L627-L648（模块级函数 L63-L78）、`commit_active` L649-L687、`_line_raw_offset` L775-L815、`on_change_draft` L815-L850、`move_left_cross` L708-L730、`move_right_cross` L730-L757 —— 均存在；`list_view` Column L2136-L2142 **无 `on_scroll`**；EditorActions 构造 L1819-L1869；`LineView(...)` 调用点 L1951-L1981；`jump_to` L1783-L1802 已有 try/except RuntimeError 模板可复用；所有新 refs/函数 **尚不存在**。
- `views/key_bindings.py`：`handle()` L80-L174；`home`/`end` 在 `_handle_edit_nav` L184-L189（`move_line_start()`/`move_line_end()` if `e.ctrl`）；标签快捷键拦截 L138-L150；`pageup`/`pagedown` **未处理**。outward_sel 顶部拦截块 L100-L136 对 `pageup`/`pagedown` 不匹配会 fall-through（不 return），故 PageUp/PageDown 在选区激活时仍可触发翻页（active is None → 走浏览态纯滚动分支，可接受）。
- `state/actions.py`：`move_line_start`/`move_line_end` **存在**（L44-L45）；`page_up`/`page_down`/`move_doc_start`/`move_doc_end` **不存在**，需新增/重命名。

## 实现步骤

### 1. 点击命中中点吸附 — `views/line_view.py`

改写 `_hit_test_segs`（L95-L124）的逐字逼近循环（L116-L122）：遍历前缀宽度，当 `local_x` 落入字符 `j-1` 的宽度区间 `[prev_w, cur_w]` 时，比较 `local_x` 与中点 `(prev_w+cur_w)/2`：左半 → 边界 `j-1`（光标在该字符前），右半 → 边界 `j`（光标在该字符后）。
- 修复最左边缘 `local_x=0` → 中点 `(0+w1)/2 > 0` → 落左半 → `disp_off=0`（原为 1，off-by-one）。
...
- 改写后循环骨架：
  ...
  for j in range(1, len(display) + 1):
      ...
      prev_w = cur_w
  ```
- 同法改写 `_hit_test_code`（L127-L161）的列号逼近（L150-L157）：用 `prev_w`/`cur_w` 中点比较替换 `>= local_x` 阈值。
- 新增 `_hit_test_x(line, x, base, line_height)`：y 无关变体，直接 `return _hit_test_segs(line, 0, len(line.segments), x, 0.0, base, line_height)`，供跨行拖拽复用（紧随 `_hit_test_segs` 之后定义）。

### 2. 拖拽跨行 x 跟踪 — `views/line_view.py` + `views/editor.py`

- `LineView`（L294-L323）增加 prop `on_hit_test_x: Callable[[int, float], int] | None = None`（`(target_li, x) -> raw_offset`）。
- `_pan_target_off`（L727-L741）跨行分支（L738-L741）：`target_li != line_idx` 时，若 `on_hit_test_x` 可用则 `t_off = on_hit_test_x(target_li, pos.x)`（用目标行 + 同一 x 列命中），否则回退现有 999999/0。
...
- 所有 `LineView(...)` 调用点（L1951-L1981）传 `on_hit_test_x=_hit_test_line_x`。

### 3. 导航滚动入视 + 视口跟踪 — `views/editor.py`

- 新增 refs：`scroll_offset_ref`、`viewport_h_ref`、`max_scroll_ref`（均 `ft.use_ref(0.0)`，与现有 refs 同区放置）。
- `list_view` Column（L2136-L2142）加 `on_scroll=_on_scroll`：
  ```python
  def _on_scroll(e):
...
- 新增 `_safe_scroll_to(li)`（复用 `jump_to` L1783-L1802 的 try/except RuntimeError 模板，防重挂载卸载报错）。
...
- `key_bindings.py` `handle()`：在标签快捷键拦截块（L138-L150）之后、`layer` 判定（L152）之前加：
  ```python
  ...
  ```
  （`page up`/`page down` → norm `pageup`/`pagedown`，两层均生效。outward_sel 激活时 fall-through 到此，active is None → 浏览态纯滚动，可接受。）
- `state/actions.py`：`EditorActions` 增 `page_up: Callable[[], None]` / `page_down: Callable[[], None]` 字段。

### 6. Ctrl+Home/Ctrl+End → 文档首/尾 — `views/editor.py` + `views/key_bindings.py` + `state/actions.py`

- `editor.py`：`move_line_start`（L600-L611）→ 重命名 `move_doc_start`，体跳文档首行（`_goto(0, seg_idx=0, cursor_at=0)`，围栏走 `_goto_quiet`）；`move_line_end`（L613-L625）→ `move_doc_end`，体跳文档末行末段段尾。均清空 `preferred_col` + `_ensure_visible`。
- `key_bindings.py` `_handle_edit_nav`（L184-L189）：`norm=="home": actions.move_doc_start() if e.ctrl else actions.move_home()`；`norm=="end": actions.move_doc_end() if e.ctrl else actions.move_end()`。
- `state/actions.py`：字段 `move_line_start`/`move_line_end` → `move_doc_start`/`move_doc_end`。
- `editor.py` EditorActions 构造（L1819-L1869）：同步重命名 + 增 `page_up=page_up, page_down=page_down`。
...
- **`views/key_bindings.py`**：`handle()` 顶部 `pageup`/`pagedown` 拦截；`_handle_edit_nav` 的 `home`/`end` Ctrl 分支改 `move_doc_start`/`move_doc_end`。
...
- outward_sel 激活时按 PageUp/PageDown 仅滚动视口，不移动选区末端（v1 可接受）。
