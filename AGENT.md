...
- 禁止把 `ft.IconButton` / `ft.Dropdown` 放进需要紧凑行高的工具栏：**行高 = 最高子项**，二者有 Material 固有高度且**压内边距无效** —— `IconButton` 默认 40（`visual_density=COMPACT` 也只降到 32），`Dropdown` 恒为 48（即使 `dense=True` / `text_size=12` / `content_padding` 归零，内部 `InputDecorator` 的高度不减）。给 `Dropdown` 设 `height=` 能改外框但**会裁切内部文字**。紧凑替代：图标按钮用固定尺寸 `Container(ink=True, width/height=22, tooltip=...)`（见 `views/status_bar.py`），语言/选项选择用自绘触发器的 `PopupMenuButton`（见 `views/code_block.py`）。
