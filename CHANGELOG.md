...
增加关于（版本、联系邮箱、官方社群、用户手册）」。

- **关闭按钮固定（结构保证，不是视觉补丁）**：改前头部（标题 + 关闭按钮）本身就在滚动 `Column`
  里，快捷键页近百行，往下滚一点关闭按钮就滚出视口，用户必须滚回顶部才能关窗。现在对话框右列是
  `[固定头部, 分隔线, 唯一滚动区]`，头部与分隔线在滚动区**之外**——关闭按钮在物理上不可能随内容移动。
- **顺带修掉一个真机才暴露的真 bug**：拆掉快捷键页原有的「内层滚动」后，外层列漏写 `expand=True`
  → 该列按内容取高，内层滚动列拿不到**有界高度**：内容被卡片裁掉**且滚轮完全无效**（真机实测滚
  6 次内容零位移）。父级 `Row` 的交叉轴约束是「至多 740」，只有 `expand` 才把它变成紧约束。
  修复：外层列与内层滚动列都显式 `expand=True`。
- **单层滚动**：五个 tab 的内容统一由右列这一个滚动区承载，不再有嵌套滚动条（嵌套滚动条既难用
  又难对齐滚动位置）。
- **排版重做**：设置项从「裸 `Text` + `Divider` 平铺」改为**语义分组卡片**（细边框 + 圆角 +
  可选标题/说明），卡片间距统一 `Spacing.XXL`；新增统一行工厂 `_switch_row` / `_slider_row` /
  `_dropdown_row` / `_value_row`，标签列与数值列定宽，**各行左右边缘对齐**；新增小圆角角标
  `_badge`。侧栏导航项改为**整行可点**（改前把 `TextButton` 嵌在 `Container` 里，只有文字那
  一点能点，行内其余位置点不动）。
- **图标按钮统一为定尺寸 `Container(ink=True)`**：全部撤掉 `ft.IconButton`（Material 固有高 40，
  `visual_density=COMPACT` 也只降到 32，压内边距无效），与顶栏 / 状态栏同一惯例。
- **快捷键「已修改」标记**：以 `ACTION_REGISTRY` 的 `default` 为基准逐项比对，偏离默认的动作名旁
  打**已修改**角标，清空绑定的打**已清空**（红色）；每个分类标题汇总「N 项已调整」；顶部概览条
  给出「已调整 N 项 / 共 M 项」。**比对前先 `normalize`**——否则 `ctrl+comma` 与 `ctrl+,` 这类
  等价写法会被误判成「已修改」（有别名 / 等价拼写用例守护）。另加「只看已修改」开关（面板内部
  `use_state`，不引入 App 级状态）。被清空的项键位框显示**未绑定**而非回退显示默认键（否则
  「已清空」角标旁边写着 `Ctrl+S`，自相矛盾），默认键改在说明行以「默认 Ctrl+S」给出。
- **新增「关于」页**（替换原先占位的「高级」tab）：版本 / 运行环境 / 作者 / 开源许可 + 联系邮箱 /
  官方社群 / 用户手册 + 相关链接；每行提供**复制**与**在浏览器中打开**，邮箱行自动转 `mailto:`。
- **新增 `config/app_meta.py`（应用身份与外链的唯一来源）**：版本走 `importlib.metadata.version`，
  源码直跑时回退 `_FALLBACK_VERSION`；名称 / 标语 / 作者 / 许可与 5 个外链常量集中一处，
  「关于」页与全局「帮助」菜单**共用同一来源**——外链一旦分散，没有任何测试能发现两处不一致。
  `views/global_menu.py` 里 6 个硬编码 URL 已改为引用它。
- **修正**：`app/_settings_controller.py` 的 `reset_shortcuts()` 原本会跳到已删除的 `"advanced"`
  tab（静默落到编辑 tab），改为跳 `"shortcuts"`。
- 新增 `tests/test_settings_dialog.py`（31 项）：关闭按钮在滚动区之外（**对五个 tab 参数化**）+
  正文在滚动区之内 + 全窗只有一个滚动容器；改动标记（等价拼写不标记 / 别名不标记 / 清空 /
  只看已修改过滤 / 分类分组）；关于页必备行与取值来自 `app_meta`；邮箱 `mailto:` 与社群外链打开；
  版本复制；tab 定义一致性；每个 tab 都能渲染出自己的说明；代码中不再引用已删除的 `advanced` tab。
- 变异验证：分别把关闭按钮挪回滚动区、去掉改动标记逻辑，对应护栏测试各自精确失败一次，
  证明断言不是恒真。
- 全量回归：**1492 passed**（较设置面板改动前 1461 净增 31，即新增测试数，无回归）。
  注：受限环境下 `tmp_path` 类用例偶发 `PermissionError`，且宿主删除守卫会对测试内的批量删除
  抛 `SystemExit(1)` —— 均为环境性，与应用代码无关。
- 真机验证（`_probe_drive2.py` 驱动真实窗口）：内容区发生滚动的那次运行里，**关闭按钮区与头部区
  逐像素全等**（`close diff=None head diff=None`），且内容无卡死假死、`ScrollMode` 生效。
  注：本会话的 OS 级输入注入不稳定（同参数时灵时不灵），该结论取自确实注入成功的那一次运行。
...
`[toc]` 卡片的条目文字**都是同一种正文色**（`c.text` / `#1F2329`），级别只体现在
左侧 3px 色条上——在侧栏扫到一条时，无法凭颜色认回正文里对应的是哪一级标题。

- **改法（同源而非同值）**：新增 `styles.heading_text_color(level, c)` 作为**标题
  文字色的唯一取色口**；编辑区（`views/segment_view.py` 3 处 + `views/rendered_line.py`
  1 处）、侧栏大纲（`views/toc.py`）、文档内 `[toc]` 卡片（`views/line_view.py`）
  共 6 处统一改走它。于是「大纲这一条 ↔ 正文那行标题」颜色一致由**同源**保证，
  不靠两边各自维护对照表；亮/暗主题各自的六级色阶仍由 `Colors.heading_colors`
  提供，切主题时六处一起变。
- **色条与文字同色**：色条不再单独取色（原来 `.get(level, c.muted)`，与文字是两套
  回退值），现在就是同一颜色的短竖线强调。
- **字重同序**：新增 `styles.outline_heading_weight(level)` —— 编辑区字重**整体降
  一档**（800→700→…→500→400）。编辑区那套字重是按 30→15px 字号设计的，大纲条目
  只有 12px，照搬 W_800 会糊；降档后「大纲里的粗细顺序」与正文逐级一致，相邻级别
  仍差 ≥100（不产生同重级别）。原先「H1/H2 加粗、其余常规」的两档粗分法随之删除，
  两处调用点各写一遍的规则合并为一处。
- **真机复验**（真实 App + 六何级标题文档，`PrintWindow` 整窗抓屏 + 逐行墨迹取色，
  dpr=1.75）：大纲 7 条标题的最深色由清一色 `(31,35,41)` 变为
  `#1A4480` / `#2C7BE5` / `#0E7C66` / `#6B5B95` / `#B54708` / `#5C6573`（末条 H2 重复），
  与编辑区渲染出的 `c.heading_colors[level]` **逐级一致**；文档内 `[toc]` 卡片同步。
- 新增 `tests/test_outline_color.py`（13 项，亮/暗各半）锁定三条不变式：**颜色同源**
  （比对编辑区**真渲染输出** `raw_to_visible_spans`，不是「两边都读同一常量」的同义
  反复，故「把编辑区标题改个色」会立刻让它失败）、**字重逐级 -100**、**两处一致**
  （`[toc]` 卡片 == 侧栏大纲）。变异验证：把大纲文字色改回 `c.text`，6 项立刻失败。
- 功能零变化：布局、缩进、点击跳转、悬停与条目顺序均未改，只换取色来源。
