...
- **行内格式快捷键**：`Ctrl+B` 加粗、`Ctrl+I` 斜体、`Ctrl+Shift+H` 高亮、`Alt+Shift+5` 删除线、`` Ctrl+` `` 行内代码、`Ctrl+K` 链接；选中文本自动包裹对应语法并**保持选区**，再次按下同一快捷键取消语法标记（Toggle 行为），无选中插入空语法标记（光标落标记中间）；浏览态无需先进入编辑态即可生效，渲染态同段选区同样支持包裹
...
对标 VS Code 内置源代码管理，命令层独立封装（`services/git/`），不侵入编辑能力：

- **侧边栏 Git 面板**：活动栏图标（带待提交变更角标）打开独立面板，分「暂存的更改 / 更改」两个可折叠分区，按 Git 原生状态标记字母与颜色（新增 `A` / 修改 `M` / 删除 `D` / 重命名 `R` / 冲突 `U` / 未跟踪 `??`）；分区标题栏提供「全部暂存 / 全部取消暂存 / 丢弃所有工作区更改」，单文件行提供「暂存 / 取消暂存 / 丢弃」；顶部支持文件名过滤
- **内嵌差异视图**：点击文件在编辑器之上唤起覆盖式 diff（不占用标签、不影响文档状态），行级增删高亮 + 行号栏；**统一 / 分栏**两种视图一键切换（选择持久化）；点击差异行号即跳到编辑器中对应行（`git_diff_meta.jump_path` 缺失——如历史里已删除的文件——时自动禁用并给出提示）
- **提交与同步**：面板顶部多行提交信息框 +「提交暂存区」「提交所有更改」+「提交后自动推送」开关；Fetch / Pull / Push 与领先 / 落后计数；提交成功后清空输入框；「撤销上次提交」把改动回退到工作区（保留暂存状态可选）
- **冲突处理**：合并 / 变基中把冲突文件标红并置顶，点击跳到编辑器中的 `<<<<<<<` 标记处，面板提供「中止合并 / 中止变基」；处于操作中时切换分支 / 拉取 / 合并 / 丢弃一律被守卫拦下并提示
- **历史记录**：按时间倒序的提交列表（短哈希 / 作者 / 相对时间 / 摘要），点击展开该提交的变更文件列表并可查看单文件历史 diff；支持按作者 / 关键词 / 文件名筛选（文件名筛选即「单文件专属修改历史」），分页加载（`git_history_page_size`，默认 50 条）；标签与文件树右键菜单均有「查看文件历史」入口
- **分支管理**：状态栏分支 chip 与面板分支按钮唤起分支列表（本地 / 远端分组、当前分支置顶、过滤、领先 / 落后标记），支持切换 / 新建并切换 / 合并 / 删除（删除走二次确认）
- **状态栏**：显示当前分支名（分离 HEAD 时提示）、`↑待推送` / `↓待拉取` / `待提交` 数量与进行中的合并 / 变基警示；点击分支名开分支列表，点击计数打开 Git 面板
- **仓库初始化**：工作区不是 Git 仓库时面板给出「初始化仓库」一键 `git init`；未检测到 Git 时给出安装提示与「重新检测」
- **错误全覆盖**：网络 / 认证 / 权限 / 冲突 / 仓库损坏 / 非仓库 / 超时分别归类为用户可读消息（面板红条 + 状态栏 + SnackBar），并附修复建议；所有 git 调用非交互（`GIT_TERMINAL_PROMPT=0`）且带超时
...
|---|---|
| `Ctrl+s` | 保存 — 保存当前文档到磁盘 |
| `Ctrl+Shift+s` | 另存为 — 另存为新文件（Ctrl+Shift+S），保存后切换到新路径 |
| `Ctrl+o` | 打开 — 打开 Markdown 文件 |
| `Ctrl+Shift+o` | 打开文件夹 — 打开文件夹作为工作区，锚定侧边栏文件树根目录 |
| `Ctrl+n` | 新建 — 创建空白文档 |
| `Ctrl+z` | 撤销 — 回退上一笔编辑 |
| `Ctrl+y` | 重做 — 恢复最近撤销的编辑 |
| `Ctrl+Shift+z` | 重做（备用） — 兼容 VS Code 风格的重做键位 |
| `Ctrl+Shift+b` | 切换侧边栏 — 显示或隐藏侧边栏 |
| `Alt+t` | 切换主题 — 在亮色与暗色主题间切换 |
...
| `Ctrl+tab` | 下一个标签 — 切换到右侧标签（循环） |
| `Ctrl+Shift+tab` | 上一个标签 — 切换到左侧标签（循环） |
| `Ctrl+l` | 无序列表 — 将当前行切换为无序列表 |
| `Ctrl+Shift+l` | 任务列表 — 将当前行切换为任务列表项（- [ ]） |
| `Alt+c` | 切换任务状态 — 勾选/取消勾选当前任务列表项 |
| `Ctrl+Shift+m` | 公式块 — 将当前行切换为块级公式（$$...$$），并进入编辑态 |
| `Ctrl+Shift+u` | 分隔线 — 将当前行切换为分隔线 |
| `Ctrl+t` | 表格 — 将当前行切换为 2×2 表格（1 表头 + 1 数据行），并进入表格编辑态 |
...
| `Alt+enter` | 替换当前匹配 — 替换当前匹配项并跳到下一个匹配 |
| `Ctrl+Alt+enter` | 全部替换 — 替换当前文档/工作区内所有匹配项 |
...
| `Ctrl+Shift+g` | 源代码管理 — 展开侧边栏并切到 Git 面板（VSCode 风格 `Ctrl+Shift+G`） |
...
| `Ctrl+Shift+u` | 分隔线 — 将当前行切换为分隔线 |
| `Ctrl+t` | 表格 — 将当前行切换为 2×2 表格（1 表头 + 1 数据行），并进入表格编辑态 |
| `Ctrl+b` | 加粗 — 选中文本包裹 **，无选中插入空标记 |
| `Ctrl+i` | 斜体 — 选中文本包裹 *，无选中插入空标记 |
| `Ctrl+Shift+h` | 高亮 — 选中文本包裹 ==，无选中插入空标记 |
| `Ctrl+`` | 行内代码 — 选中文本包裹 `，无选中插入空标记 |
| `Ctrl+k` | 链接 — 选中文本包裹为 [text](url)，无选中插入空链接 |
| `Ctrl+m` | 行内公式 — 选中文本包裹 $，无选中插入空标记 |
| `Ctrl+d` | 删除线 — 选中文本包裹 ~~，无选中插入空标记 |
| `Ctrl+c` | 复制 — 复制选区文本到剪贴板 |
| `Ctrl+x` | 剪切 — 复制选区文本到剪贴板并删除选中内容 |
| `Ctrl+v` | 粘贴 — 在光标处插入剪贴板内容，多行自动拆分 |
| `Ctrl+Shift+v` | 纯文本粘贴 — 粘贴并剥离 Markdown 语法（MarkText 风格） |
...
>
> **Git 提交框**另有两条只在「提交信息输入框获得焦点」时生效的快捷键：`Ctrl+Enter`
> 提交暂存区、`Ctrl+Shift+Enter` 提交所有更改（VSCode 同款）。它们**刻意不设全局默认键位**
> ——否则会在编辑器里抢走 `Ctrl+Enter`（那里是「切换原文模式」）。可在「设置 → 快捷键」
> 中用 `git_commit` / `git_commit_all` 两项自定义，改后只在提交框内生效。
...
| 代码块边界方向键 | 光标在代码块第一行按 `↑`（或行首按 `←`）跳出到上一行行尾、最后一行按 `↓`（或行尾按 `→`）跳出到下一行行首（Typora 式）；代码块前/后无行（或相邻行为岛屿块）时自动创建新空行承接光标 |
| `Home` / `End` | 跳到行首 / 行尾 |
