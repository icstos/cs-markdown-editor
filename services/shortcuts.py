...
DEFAULT_SHORTCUTS: dict[str, dict[str, str]] = {
    "browse": {
        ...
        "format_table": "ctrl+t",
        "format_list": "ctrl+l",
        "format_hr": "ctrl+shift+u",
        "toggle_task": "alt+c",
        "format_task": "ctrl+shift+l",
...
#
# **不变量：注册表里的每个动作都必须有真实的分发路径**（见
# tests/test_shortcut_reachability.py）。注册表是「设置 → 快捷键」面板的数据源，
# 面板会为每一项渲染可编辑的键位框；一旦注册了不存在的动作，用户会看到
# 「Ctrl+U 下划线」这类键位并可自定义，按下去却什么都不会发生——静默失效。
# 因此历史上那些「有键位、无实现、无分发」的幽灵条目（删除行 / 复制为富文本 /
# 下划线 / 插入图片 / 清除格式，以及无默认键的标题级别条目）已全部移除；
# 需要它们时先补实现与分发，再加回注册表。
ACTION_REGISTRY: list[ActionDef] = [
    ...
    ActionDef("toggle_word_wrap", "自动换行", "both", "视图",
              "切换文档长行是否自动换行（VSCode 风格 Alt+Z）。",
    ...
    ActionDef("close_tab", "关闭标签", "browse", "视图", "关闭当前标签（全局生效，脏标签走确认）。",
              {"browse": "ctrl+w"}),
    ActionDef("reopen_closed_tab", "重新打开关闭的标签页", "browse", "文件",
              "按关闭顺序恢复最近关闭的标签（Ctrl+Shift+T，可连续按多次）。",
              {"browse": "ctrl+shift+t"}),
    ActionDef("next_tab", "下一个标签", "browse", "视图", "切换到右侧标签（循环）。",
              {"browse": "ctrl+tab"}),
    ActionDef("prev_tab", "上一个标签", "browse", "视图", "切换到左侧标签（循环）。",
              {"browse": "ctrl+shift+tab"}),
    ActionDef("format_list", "无序列表", "both", "格式", "将当前行切换为无序列表。",
              {"browse": "ctrl+l", "edit": "ctrl+l"}),
    ActionDef("format_task", "任务列表", "both", "格式", "将当前行切换为任务列表项（- [ ]）。",
              {"browse": "ctrl+shift+l", "edit": "ctrl+shift+l"}),
    ...
    ActionDef("format_hr", "分隔线", "both", "格式", "将当前行切换为分隔线。",
              {"browse": "ctrl+shift+u", "edit": "ctrl+shift+u"}),
    ActionDef("format_table", "表格", "both", "格式",
              "将当前行切换为 2×2 表格（1 表头 + 1 数据行），并进入表格编辑态。",
              {"browse": "ctrl+t", "edit": "ctrl+t"}),
    ...
    ActionDef("format_strike", "删除线", "edit", "行内格式", "选中文本包裹 ~~，无选中插入空标记。", {"edit": "ctrl+d"}),
    ...
    ActionDef("format_markdown", "格式化文档", "both", "编辑",
    ...
    ActionDef("global_find", "全局查找", "both", "视图", "在侧边栏搜索面板中跨文件查找。",
              {"browse": "ctrl+shift+f", "edit": "ctrl+shift+f"}),
