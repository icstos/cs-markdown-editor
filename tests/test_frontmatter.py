...
- paste_row_from_clipboard：剪贴板 "key: value" 解析插入（无冒号回退、
  空剪贴板 no-op）
"""
...
from services.clipboard import paste_row_from_clipboard
from views._frontmatter import parse_yaml_pairs
...
def test_parse_basic_pairs():
    """基本 key: value 解析。"""
    assert parse_yaml_pairs("title: 我的文档\ntags: note") == [
        ("title", "我的文档"),
        ("tags", "note"),
    ]
...
def test_parse_empty_and_whitespace_key():
    ...
    assert parse_yaml_pairs("") == []
    assert parse_yaml_pairs("  indented: x") == [("indented", "x")]
...
def test_paste_parses_key_value():
    ...
    inserted = []
    _run(paste_row_from_clipboard(types.SimpleNamespace(current=cb),
...
def test_paste_missing_clipboard_noop():
    ...
    inserted = []
    _run(paste_row_from_clipboard(None, lambda k, v: inserted.append((k, v))))
    assert inserted == []
