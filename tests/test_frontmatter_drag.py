...
from views._frontmatter import drag_src_idx, reorder_pairs
...
def test_src_idx_reads_draggable_data():
    ...
    assert drag_src_idx(_evt(2), 4) == 2
    assert drag_src_idx(_evt(0), 4) == 0


def test_src_idx_none_src():
    """e.src 为 None（控件解析失败）：返回 None。"""
    assert drag_src_idx(_evt(None), 4) is None


def test_src_idx_non_int_data():
    """data 非 int（异常数据）：返回 None。"""
    assert drag_src_idx(_evt("2"), 4) is None
    assert drag_src_idx(_evt(None), 4) is None  # src 存在但 data 为 None


def test_src_idx_out_of_range():
    """越界索引（拖拽期间行数变化）：返回 None 防御。"""
    assert drag_src_idx(_evt(4), 4) is None
    assert drag_src_idx(_evt(-1), 4) is None
    assert drag_src_idx(_evt(99), 4) is None


def test_src_idx_empty_pairs():
    """无行时任何索引都无效。"""
    assert drag_src_idx(_evt(0), 0) is None
...
def test_reorder_down():
    ...
    assert reorder_pairs(pairs, 0, 3) == [
        ["b", "2"], ["c", "3"], ["d", "4"], ["a", "1"],
    ]
...
def test_reorder_up():
    """行 3 拖到行 0。"""
    pairs = [["a", "1"], ["b", "2"], ["c", "3"], ["d", "4"]]
    assert reorder_pairs(pairs, 3, 0) == [
        ["d", "4"], ["a", "1"], ["b", "2"], ["c", "3"],
    ]
...
def test_reorder_middle():
    """行 1 拖到行 3。"""
    pairs = [["a", "1"], ["b", "2"], ["c", "3"], ["d", "4"]]
    assert reorder_pairs(pairs, 1, 3) == [
        ["a", "1"], ["c", "3"], ["d", "4"], ["b", "2"],
    ]
...
def test_reorder_same_position():
    ...
    pairs = [["a", "1"], ["b", "2"]]
    assert reorder_pairs(pairs, 0, 0) == pairs


def test_reorder_invalid_indices_no_change():
    """越界索引：返回原顺序副本，不抛异常。"""
    pairs = [["a", "1"], ["b", "2"]]
    assert reorder_pairs(pairs, 5, 0) == pairs
    assert reorder_pairs(pairs, 0, 5) == pairs
    assert reorder_pairs(pairs, -1, 0) == pairs


def test_reorder_does_not_mutate_original():
    """返回新列表，不修改原列表（含行对象拷贝）。"""
    pairs = [["a", "1"], ["b", "2"], ["c", "3"]]
    result = reorder_pairs(pairs, 0, 2)
...
def test_reorder_keeps_pending_empty_row():
    ...
    pairs = [["a", "1"], ["b", "2"], ["", ""]]
    assert reorder_pairs(pairs, 0, 2) == [
        ["b", "2"], ["", ""], ["a", "1"],
    ]
