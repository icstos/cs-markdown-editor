...
覆盖 views.sidebar：
- tree_signature：树内容 → 签名（文件增删/嵌套目录可区分、无变化签名稳定）
...
from services.file_tree import poll_fs_changes, scan_files, tree_signature


# ---- tree_signature：签名与树内容一一对应 ----


def test_signature_stable_without_change(tmp_path):
    """无文件变化时两次扫描签名一致。"""
    (tmp_path / "a.md").write_text("x", encoding="utf-8")
    sig1 = tree_signature(scan_files(str(tmp_path)))
    sig2 = tree_signature(scan_files(str(tmp_path)))
    assert sig1 == sig2
...
def test_signature_covers_nested_dirs(tmp_path):
    ...
    sig = tree_signature(scan_files(str(tmp_path)))
    assert "/sub" in sig
    assert "a.md" in sig
...
def test_poll_preset_base_silent(tmp_path):
    ...
    base = types.SimpleNamespace(
        current=tree_signature(scan_files(str(tmp_path)))
    )
    changes: list[int] = []
    stop = {"flag": False}
