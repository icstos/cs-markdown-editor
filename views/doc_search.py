...
复用 services.search 的行级正则匹配（build_query_regex / match_lines）：
...
from services.search import build_query_regex, match_lines
...
def compute_doc_matches(
    ...
    - 空查询 / document 为空 / 正则非法（build_query_regex 返回 None）→ []
    - 超过 limit 时截断（保前 limit 条，行号升序）
    """
    if document is None or not query.strip():
        return []
    pattern = build_query_regex(query, case_sensitive, False, regex)
    if pattern is None:
        return []
    results = match_lines(document, pattern)
